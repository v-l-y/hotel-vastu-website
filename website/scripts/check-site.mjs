...
for(const token of ["scrollIntoView","prefers-reduced-motion","requestAnimationFrame","is-scrolled","IntersectionObserver","lux-reveal","is-visible","main .room-grid>.card","main .photo-story-grid>figure","--reveal-delay"]){
  if(!mainJs.includes(token)) fail("js/main.js","missing smooth-scroll token "+token);
}
for(const token of ["translate3d(0,26px,0)","scale(.992)","will-change:opacity,transform"]){
